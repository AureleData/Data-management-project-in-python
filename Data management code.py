...
base_url = 'https://github.com/AureleData/Data-management-project-in-python/raw/main/'

# Read raw data files into DataFrames
amounts_url = base_url + 'raw%20data%20for%20services%20example.xlsx'
amounts = pd.read_excel(amounts_url, sheet_name='NEW DATA alloc')

cubealloc_url = base_url + 'raw%20data%20for%20services%20example.xlsx'
cubealloc = pd.read_excel(cubealloc_url, sheet_name='cube alloc')

ctralloc_url = base_url + 'raw%20data%20for%20services%20example.xlsx'
ctralloc = pd.read_excel(ctralloc_url, sheet_name='country alloc', keep_default_na=False)

# Read the matrix schema from an Excel file into a DataFrame
matrix_url = base_url + 'raw%20data%20for%20services%20example.xlsx'
matrix = pd.read_excel(matrix_url, sheet_name='Matrix schema')
